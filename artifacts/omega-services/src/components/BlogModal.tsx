          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              <div className="bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                {/* Scrollable content */}
                <div
                  className="p-6 overflow-y-auto flex-1 text-sm text-gray-600 leading-relaxed space-y-3"
                  dir={language === "ar" ? "rtl" : "ltr"}
                >
                  {post.content[language].split("\n\n").map((paragraph, i) => {
                    if (paragraph.startsWith("**") && paragraph.endsWith("**")) {
                      return (
                        <h3 key={i} className="font-bold text-[#1a3c6e] text-base mt-4">
                          {paragraph.slice(2, -2)}
                        </h3>
                      );
                    }
                  ...
                  <div className="flex gap-3 justify-end">
                    ...
                    <a
                      href={`https://wa.me/21654651063?text=${encodeURIComponent(
                        language === "fr"
                          ? `Bonjour, j'ai une question sur : ${post.title.fr}`
                          : `مرحبا، لديّ سؤال حول: ${post.title.ar}`
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-5 py-2.5 rounded-xl bg-[#e8801a] hover:bg-[#d0720f] text-white text-sm font-semibold transition-all hover:scale-[1.02] active:scale-[0.98]"
                    >
