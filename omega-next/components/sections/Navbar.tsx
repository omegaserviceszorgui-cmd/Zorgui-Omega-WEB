...
const searchableItems = [
  { label: { fr: "Services administratifs", ar: "خدمات إدارية" }, section: "services" },
  { label: { fr: "Sécurité sociale", ar: "الضمان الاجتماعي" }, section: "services" },
  { label: { fr: "Retraite à l'étranger", ar: "التقاعد بالخارج" }, section: "services" },
  ...
  { label: { fr: "Blog", ar: "المدونة" }, section: "blog" },
  { label: { fr: "Réserver un rendez-vous", ar: "حجز موعد" }, section: "booking" },
];
...
export default function Navbar() {
  ...
  const handleSearch = (val: string) => {
    ...
    setResults(searchableItems.filter(i =>
      i.label.fr.toLowerCase().includes(lower) || i.label.ar.includes(val)
    ));
    setShowResults(true);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="flex items-center gap-2 flex-shrink-0">
            <Image src="/images/logo.png" alt="OMEGA SERVICES" width={120} height={40} className="h-10 w-auto object-contain" />
          </button>
          ...
          <div className="flex items-center gap-3">
            <div ref={searchRef} className="relative hidden md:block">
              ...
              {showResults && results.length > 0 && (
                <div className="absolute top-full mt-1 left-0 right-0 bg-white rounded-lg shadow-xl overflow-hidden z-50">
                  {results.map((r, i) => (
