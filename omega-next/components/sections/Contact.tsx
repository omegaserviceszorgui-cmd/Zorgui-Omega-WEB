...
export default function Contact() {
  ...
  const contactInfo = [
    { icon: Phone, labelFr: "Téléphone", labelAr: "الهاتف", value: "98284858", href: "tel:+21698284858" },
    { icon: MessageCircle, labelFr: "WhatsApp", labelAr: "واتساب", value: "54651063", href: "https://wa.me/21654651063" },
    { icon: Mail, labelFr: "Email", labelAr: "البريد الإلكتروني", value: "omegaserviceszorgui@gmail.com", href: "mailto:omegaserviceszorgui@gmail.com" },
    { icon: MapPin, labelFr: "Adresse", labelAr: "العنوان", value: "Kasserine – Tunisie", href: "https://maps.google.com/?q=Kasserine,Tunisia" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <p className="text-gray-500 text-lg">
            {language === "fr" ? "Nous sommes à votre disposition du lundi au samedi" : "نحن في خدمتكم من الاثنين إلى السبت"}
          </p>
          <div className="mt-4 w-16 h-1 bg-[#e8801a] mx-auto rounded-full" />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-[#1a3c6e] to-[#0d2140] rounded-2xl p-6 text-white">
              <h3 className="font-bold text-lg mb-2">{language === "fr" ? "Horaires d'ouverture" : "ساعات العمل"}</h3>
              <div className="space-y-1 text-white/80 text-sm">
                <div className="flex justify-between"><span>{language === "fr" ? "Lun – Ven" : "الاثنين – الجمعة"}</span><span className="font-semibold text-white">08:00 – 18:00</span></div>
                <div className="flex justify-between"><span>{language === "fr" ? "Samedi" : "السبت"}</span><span className="font-semibold text-white">09:00 – 14:00</span></div>
                <div className="flex justify-between"><span>{language === "fr" ? "Dimanche" : "الأحد"}</span><span className="font-semibold text-[#e8801a]">{language === "fr" ? "Fermé" : "مغلق"}</span></div>
              </div>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-[#1a3c6e] mb-1.5">{t("booking.name")} *</label>
                  <input type="text" required value={form.name} onChange={e => set("name", e.target.value)}
                    className={inputClass} placeholder={language === "fr" ? "Votre nom" : "اسمك"} />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-[#1a3c6e] mb-1.5">{t("contact.subject")}</label>
                  <input type="text" value={form.subject} onChange={e => set("subject", e.target.value)}
                    className={inputClass} placeholder={language === "fr" ? "Sujet de votre message" : "موضوع رسالتك"} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-[#1a3c6e] mb-1.5">{t("contact.message")} *</label>
              ...
              <button type="submit" disabled={status === "loading"}
                className="w-full bg-[#e8801a] hover:bg-[#d0720f] disabled:bg-gray-300 text-white font-semibold py-3.5 rounded-xl transition-all hover:scale-[1.01] shadow-lg text-sm">
                {status === "loading" ? t("form.loading") : t("contact.submit")}
              </button>
              {status === "success" && (
